...
import glob
from dataloader_new import AinoZoo

class CycleGANModel(nn.Module):
    def __init__(self, opt, datasetA=None, datasetB=None):
        super(CycleGANModel, self).__init__()

        self.opt = opt
        #for denormalization
        self.datasetA: AinoZoo = datasetA
        self.datasetB: AinoZoo = datasetB
        ...
        if self.isTrain:
            ...
            if opt.optimizer=="Adam": # Adam
                ...
                if self.opt.temporal_discriminator: # True
            ...
            self.criterionEndeffector = EndEffectorLoss(opt.remove_virtual_node==1,opt.window_size,loss_type=opt.ee_loss_type,\
                                                        with_root=self.opt.with_root,datasetA=self.datasetA, datasetB=self.datasetB,\
                                                        velocity_virtual_node=self.opt.velocity_virtual_node,\
                                                        foot_height_A=self.datasetA.foot_height,\
                                                        foot_height_B=self.datasetB.foot_height,\
