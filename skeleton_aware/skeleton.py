...
class SkeletonConv(nn.Module):
    ...
    def forward(self, input):
        ...
        # print("input", input.shape, "weight", weight_masked.shape, "bias", self.bias.shape)
...
class SkeletonLinear(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, extra_dim1=False):
        ...
        # print(self.weight.shape,"in_channels",in_channels,"out_channels",out_channels)
    ...
    def forward(self, input):
        # print("input shape of SkeletonLinear layer",input.shape,"weight shape",self.weight.shape,"bias shape",self.bias.shape)
        input = input.reshape(input.shape[0], -1)
        # print(input.shape)
        weight_masked = self.weight * self.mask
        # print("res",F.linear(input, weight_masked, self.bias).shape)
...
class SkeletonPool(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        # print("input shape",input.shape,"weight shape",self.weight.shape)
