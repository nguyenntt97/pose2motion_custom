...
class AinoZoo(torch.utils.data.Dataset):
    ...
    #     std_bvh_path = bvh_path.parent / "std_bvhs" / f"{bvh_path.name}.bvh"
    #     print("Standardized BVH path:", std_bvh_path)
    #     return std_bvh_path
    
    # def _get_motion(bvh_path: Path):
    #     return bvh_path.parent / (bvh_path.name + ".npy")
    ...
    def _load_dataset(self):
        ...
        for sub_dir in sub_dirs:
            ...
            for subject in subject_dirs:
                ...
                if subject_name not in ["coyote", "labrador"]:
                    continue
                ...
                if self.use_velo_virtual_node:
                    ...
                    gt_root_velo = root_velo # TODO: fix this
            ...
            #     # bvh_data = self._load_bvh(f)
            #     bvh_data = BVH_file(AinoZoo._get_std_bvh(f))
    ...
    def _calc_t_pose(joint_num: int, offsets: NDArray[np.float32], joint_parents_idx: NDArray[np.int32]):
        t_pose = np.zeros((joint_num, 3), dtype=np.float32)
        for j in range(joint_num):
            p = joint_parents_idx[j]
            if p == j: # root
                t_pose[j] = offsets[j]
            else:
                t_pose[j] = t_pose[p] + offsets[j]
        return t_pose
